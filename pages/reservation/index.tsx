...
import { Card, Button, Row, Col, Badge, Container, Stack, Spinner } from "react-bootstrap";
...
const VehicleReservationPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const [fetchedVehicles, fetchedAgencies] = await Promise.all([
        fetchVehicles(),
        fetchAgencies(),
      ]);
  ...
  const available = vehicles.filter(
    (v) => v.available && getStatus(v) === "available"
  ).length;
  const unavailable = vehicles.filter(
    (v) => !v.available || getStatus(v) === "unavailable"
  ).length;
  ...
  const handleCarpooling = async (id_trip: number) => {
    const goCarpooling = await fetch(
      `${process.env.backendAPI}/api/carpooling`,
      {
        ...
        body: JSON.stringify({
          id_passenger: Number(JSON.parse(Cookies.get("user")).id),
          id_trip: id_trip,
        }),
      }
    );
  ...
  const sendReservation = async (info: typeof reservationInfo) => {
    ...
    // Vérifier que le véhicule n'est pas déjà réservé aux mêmes dates
      ...
      if (
        ...
        if (
          (reservationInfo.startDate &&
            tripStart <= reservationInfo.startDate &&
            tripEnd >= reservationInfo.startDate) ||
          (reservationInfo.endDate &&
            tripStart <= reservationInfo.endDate &&
            tripEnd >= reservationInfo.endDate)
        ) {
    ...
    const payload = {
      ...
      start_date: reservationInfo.startDate
        ? reservationInfo.startDate.toISOString()
        : null,
      end_date: reservationInfo.endDate
        ? reservationInfo.endDate.toISOString()
        : null,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!vehicleCleanlinessState.ok) {
          ...
          toast.error(
            `Erreur lors de la mise à jour de l'état du véhicule: ${data.message}`
          );
        } else {
        ...
        <Row className="mb-4 g-3">
          <Col xs={6} md={4}>
            <Card className="text-center border-0 shadow-sm bg-light">
              <Card.Body className="d-flex flex-column align-items-center py-4">
                <div
                  className="rounded-circle bg-purple bg-opacity-10 d-flex align-items-center justify-content-center mb-2"
          ...
          <Col xs={6} md={4}>
            <Card className="text-center border-0 shadow-sm bg-light">
              <Card.Body className="d-flex flex-column align-items-center py-4">
                <div
                  className="rounded-circle bg-secondary bg-opacity-10 d-flex align-items-center justify-content-center mb-2"
                  style={{ width: 48, height: 48 }}
                >
            ...
            {carpoolTrips.length > 0 ? (
                    ...
                    <Card className="h-100 shadow-sm border-0 bg-light">
                      ...
                      <Card.Body>
                        ...
                        <div className="mb-1">
                          ...
                          {vehicle.seat_count -
                            1 -
                            (vehicle.trips?.find((t) => t.id_trip === id_trip)?.carpoolings?.length || 0)}
                        </div>
                      </Card.Body>
                      <Card.Footer className="bg-white border-0">
                        {(() => {
                          const trip = vehicle.trips?.find((t) => t.id_trip === id_trip);
                          const maxSeats = vehicle.seat_count - 1; // 1 seat pour le conducteur
              ...
              <Col>
                <Card className="text-center shadow-sm border-0">
                  <Card.Body>
                    <p className="mb-0">
                      Aucun trajet disponible au covoiturage actuellement.
                    </p>
                  </Card.Body>
                </Card>
              </Col>
                  ...
                  <Form.Group controlId="reservationStartDate">
                    ...
                    <DatePicker
                      ...
                      onChange={(date: Date | null) =>
                        setReservationInfo({ ...reservationInfo, startDate: date })
                      }
                      showTimeSelect
                      timeFormat="HH:mm"
                      timeIntervals={15}
                ...
                <Col md={6}>
                  <Form.Group controlId="reservationEndDate">
                    ...
                    <DatePicker
                      ...
                      onChange={(date: Date | null) =>
                        setReservationInfo({ ...reservationInfo, endDate: date })
                      }
                      showTimeSelect
                      timeFormat="HH:mm"
                      timeIntervals={15}
